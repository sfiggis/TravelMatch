...
RSpec.describe "SearchRequest", :type => :request do
  describe "search api request" do
    ...
    it "creates a search and routes" do
      # stub(current_traveller: traveller, authenticate_traveller!: true)
      # login_as traveller, scope: :traveller
      post "/searches", :params => { search: { departureDate: Date.new(2017,11, 01), returnDate: Date.new(2017,11, 03), origin: "MOW", travellerId: traveller.id, budget: 300}, session: {:current_traveller => traveller.id} }
